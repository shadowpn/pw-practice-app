...
export class NavigationPage { //Определяет класс NavigationPage, который будет экспортирован для использования в других частях теста.
    ...
    async toastrtPage() {
         ...
         await this.toastrMenuItem.click() //Аналогично, ищет и кликает по элементу с текстом "Toastr". Это может быть действие, которое ведет пользователя к функциональности, связанной с уведомлениями (toasts).
    }
    async tooltipPage(){
          await this.selectGroupMenuItem("Modal & Overlays") //Ищет на странице элемент с текстом "Modal & Overlays" и кликает по нему. Это обычно используется для перехода на другую часть сайта или открытия модального окна.
          await this.tooltipMenuItem.click() //Аналогично, ищет и кликает по элементу с текстом "Toastr". Это может быть действие, которое ведет пользователя к функциональности, связанной с уведомлениями (toasts).
    }
